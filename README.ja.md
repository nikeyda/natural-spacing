...
`naturalLanguage` ポリシーを有効にすると、次のように変換されます。
...
フォーマッターが空白を挿入するのは、漢字とラテン文字、または漢字と ASCII 数字が直接隣接する境界だけです。既存の空白、句読点、`React18` のようなラテン文字と数字の並び、および `verbatim` ポリシーを使用するフィールドは変更されません。ライブエディター用アダプターは、IME の変換中テキスト、選択範囲、削除意図、ネイティブの Undo 動作を維持しながら、同じ見た目の結果を生成するよう設計されています。現在の実験的なサポートレベルは以下を参照してください。
