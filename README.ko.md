...
`naturalLanguage` 정책을 활성화하면 다음과 같이 변환됩니다.
...
포매터는 한자와 라틴 문자 또는 한자와 ASCII 숫자가 직접 맞닿는 경계에만 공백을 삽입합니다. 기존 공백, 문장 부호, `React18`과 같은 라틴 문자와 숫자 조합, 그리고 `verbatim` 정책을 사용하는 필드는 변경하지 않습니다. 실시간 편집기 어댑터는 IME 조합 입력, 선택 영역, 삭제 의도, 네이티브 실행 취소 동작을 보존하면서 동일한 화면 결과를 만들도록 설계되었습니다. 현재의 실험적 지원 수준은 아래에서 확인할 수 있습니다.
