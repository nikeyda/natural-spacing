...
namespace NaturalSpacing.Wpf;

/// <summary>
/// Experimental settled-input adapter for a plain-text WPF <see cref="TextBox"/>.
/// </summary>
public sealed class NaturalSpacingTextBoxAdapter : IDisposable
{
        ...
        NaturalSpacing.Core.FieldPolicy policy = NaturalSpacing.Core.FieldPolicy.Verbatim)
    {
        ...
        TextCompositionManager.AddPreviewTextInputStartHandler(_textBox, OnTextInputStarted);
        TextCompositionManager.AddPreviewTextInputHandler(_textBox, OnTextInputCompleted);
    }
    ...
    public void Dispose()
    {
        ...
        _isDisposed = true;
        _textBox.TextChanged -= OnTextChanged;
        TextCompositionManager.RemovePreviewTextInputStartHandler(_textBox, OnTextInputStarted);
        TextCompositionManager.RemovePreviewTextInputHandler(_textBox, OnTextInputCompleted);
    }

    private void OnTextInputStarted(object sender, TextCompositionEventArgs args) =>
        _isComposing = true;

    private void OnTextInputCompleted(object sender, TextCompositionEventArgs args)
    {
        _isComposing = false;
